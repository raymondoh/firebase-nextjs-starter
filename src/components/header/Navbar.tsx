...
//         ) : (
//           <>
//             {/* Only show these in mobile view since desktop shows them in the header */}
//             {isMobile && (
//               <>
...
import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";
import { useSession, signOut } from "next-auth/react";
import { generalNavItems, adminNavItems } from "@/lib/navigation";
import { Skeleton } from "@/components/ui/skeleton";
import { toast } from "sonner";
import {
  DropdownMenu,
...
const UserMenu = ({ isMobile }: { isMobile?: boolean }) => {
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      if (document.cookie.includes("account-deleted=")) {
        document.cookie = "account-deleted=; Max-Age=-1; path=/;";
        window.location.reload();
      }
    }
  ...
  useEffect(() => {
    const now = Date.now();
    if (
      isUpdatingSessionRef.current ||
      sessionCheckRef.current ||
      now - lastSessionCheckTimeRef.current < SESSION_THROTTLE_MS
    )
      return;
    sessionCheckRef.current = true;
    lastSessionCheckTimeRef.current = now;
