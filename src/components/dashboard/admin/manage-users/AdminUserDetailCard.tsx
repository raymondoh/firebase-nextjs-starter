...
//                 {/* <p className="text-sm text-muted-foreground">
//                   {formatDate(
//                     formData.lastLoginAt && typeof formData.lastLoginAt === "object" && "toDate" in formData.lastLoginAt
//                       ? formData.lastLoginAt.toDate()
...
// Using formatDate to display dates
import { formatDate } from "@/utils/date";
...
export function AdminUserDetailCard({ user }: AdminUserDetailCardProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  // Initialize formData using serializeData to normalize timestamps.
  const [formData, setFormData] = useState<SerializedUser>(() => serializeData(user) as SerializedUser);
  const { data: session } = useSession();
          ...
          <Tabs defaultValue="details" className="w-full">
            ...
            <TabsContent value="activity" className="space-y-4 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <h4 className="text-sm font-medium">Created At</h4>
                  <p className="text-sm text-muted-foreground">{formatDate(formData.createdAt, { relative: true })}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium">Last Login</h4>
                  <p className="text-sm text-muted-foreground">
